...
class Encoder(nn.Module):
    def __init__(self, mod, d_model, N, heads, dropout, input_size, customAttn= True):
        ...
        self.mod = mod
        self.Num_layers = N
        if mod == 'rec': ####### Control the high noise realizations ##############
        ...
        if mod == 'rec2': ####### Control the high noise realizations ##############
            self.FC1 = nn.Linear(input_size, d_model*2, bias=True)
            self.FC2 = nn.Linear(d_model*2, d_model*2, bias=True)
            self.FC3 = nn.Linear(d_model*2, d_model, bias=True)
            self.activation1 = nn.GELU()
            self.activation2 = nn.GELU()
        else: ############ for trx we do not need noise supression ################
            self.FC = nn.Linear(input_size, d_model, bias=True)
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, flag_TX,  dropout=0.0, norm_first = True, custom_attn = True):
        ...
        self.ffNet = FeedForward(d_model, d_ff=4 * d_model, dropout=dropout, act='gelu')# linear -> act -> drop ->linear
...
class FeedForward(nn.Module):
    def __init__(self, d_model, d_ff=2048, dropout=0.0, act = 'gelu'):
